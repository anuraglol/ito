...
<template>
  <div class="relative w-full overflow-hidden">
    <div
      ...
      :key="presence.userId"
      class="pointer-events-none fixed z-50 transition-all duration-75 ease-out flex items-center gap-1.5"
      :style="{
        left: `${presence.x}px`,
        top: `${presence.y}px`,
      }"
    >
      ...
      <span
        class="px-1.5 py-0.5 rounded text-[10px] text-white font-medium shadow-sm"
    ...
    <div class="flex flex-col gap-4 w-full">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div
            class="flex items-center gap-1 border-l pl-3 border-neutral-200 dark:border-neutral-800"
          >
            <span class="text-xs text-neutral-400">Editing as:</span>
            <span
                  ...
                  <div class="flex flex-col gap-1 w-full">
                    <div class="flex items-center justify-between gap-2 mb-1">
                      <div class="flex items-center gap-1 min-w-0">
                        ...
                        <UBadge
                          v-if="item.tag || item.priority"
                          :label="item.tag || item.priority"
                          :color="
                            item.tagColor ||
                      ...
                      <UTooltip text="delete the issue">
                        <UButton
                          ...
                          variant="ghost"
                          color="neutral"
                          @click.stop="onDeleteTask(item.id, item.version)"
                        />
                      </UTooltip>
                    </div>
                    ...
                    <div v-if="item.createdById" class="text-[10px] text-neutral-400 pt-1">
                      by {{ item.createdById.slice(0, 5) }}
                    </div>
                  </div>
                </UCard>
              </div>
